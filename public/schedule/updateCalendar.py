...
def update_calendar(hw_file, lab_file, mp_file, quiz_file, lecture_file, calendar_file, output_schedule_file):
    ...
    def get_quiz_events_by_date(quiz_events):
        event_dict = {}
        for quiz in quiz_events:
            start_date = datetime.strptime(quiz['date'], "%Y-%m-%d")
            due_date = datetime.strptime(quiz['due_date'], "%Y-%m-%d")
