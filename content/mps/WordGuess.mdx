...
The final version of this MP is a combined part 1 and part 2 where the user can select which mode they want to play, aka normal mode or evil mode.
...
It then picks the "'e' in spot 5" group because it has the most potential words, aka 3. The algorithm tells the user their guess was correct and updates the state to show the 'e' in the last position.
...
It then picks the "No 'n'" group because it has the most potential words. The algorithm tells the user their guess was incorrect and still shows the user "_ \_ \_ \_ \_ e" as the state.
